...
# Επιτρέπει σε κάποιες "κομμένες" εικόνες να διαβαστούν
...
def load_image_from_upload(uploaded_file):
    """
    Προσπαθεί να διαβάσει την εικόνα όσο πιο ανθεκτικά γίνεται.
    Επιστρέφει PIL Image ή σηκώνει εξαίρεση.
    """
    ...
    # Δεύτερη προσπάθεια: ξαναδοκιμή με verify + reopen
    try:
        buf = io.BytesIO(raw)
        img = Image.open(buf)
        img.verify()
        ...
        raise UnidentifiedImageError(f"Το αρχείο δεν αναγνωρίζεται ως εικόνα: {uploaded_file.name}") from e
        ...
        for file in uploaded_files:
            try:
                if file.size > MAX_SIZE:
                    st.error(f"⚠️ Το αρχείο '{file.name}' είναι πολύ μεγάλο. Μέγιστο επιτρεπτό: 15MB.")
                    continue

                # Διάγνωση
                st.write(f"📄 `{file.name}` | τύπος: `{file.type}` | μέγεθος: `{file.size} bytes`")

                pil_image = load_image_from_upload(file)
                ...
                st.error(
                    f"⚠️ Το αρχείο '{file.name}' δεν αναγνωρίζεται ως εικόνα. "
                    "Πιθανότατα είναι αλλοιωμένο ή δεν είναι πραγματικό PNG/JPG."
                )
            except Exception as e:
                ...
                with st.spinner("Ανάγνωση οθόνης συστήματος και σύγκριση..."):
                    try:
                        if wt_screen_file.size > MAX_SIZE:
                            st.error(f"⚠️ Το αρχείο '{wt_screen_file.name}' είναι πολύ μεγάλο. Μέγιστο επιτρεπτό: 15MB.")
                        else:
                            pil_wt = load_image_from_upload(wt_screen_file)
